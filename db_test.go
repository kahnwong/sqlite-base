...
func TestOpen_CreatesTablesOnNewDatabase(t *testing.T) {
	t.Parallel()
	...
	db, err := Open(Config{
		...
		Tables: []TableDefinition{
			{Name: "users", CreateSQL: "CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT NOT NULL)"},
		},
	})
	if err != nil {
...
func TestOpen_DoesNotRecreateTablesOnExistingDatabase(t *testing.T) {
	t.Parallel()
	...
	first, err := Open(Config{
		Path:   dbPath,
		Tables: []TableDefinition{{Name: "items", CreateSQL: "CREATE TABLE items (id INTEGER PRIMARY KEY)"}},
	})
	if err != nil {
	...
	if err != nil {
		t.Fatalf("second open should not rerun create sql: %v", err)
	}
